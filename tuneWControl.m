...
%c2d process.
tfcd = kc*tf(1,[1/(wn*wn) (2.0*zeta)/wn 1])*tf(1,[1 0],'inputdelay',delay);
Ac=[-2.0*zeta*wn -1.0*wn*wn 0;1 0 0;0 1 0];
Bc=[kc*wn*wn;0;0];
Cc=[0 0 1];
Dc=[0];
ssc = ss(Ac,Bc,Cc,Dc);
...
if ptam_on
    ...
    measuNoise = 0.0624;   %i.e. using Vicon so very accurate
...
%State and input weightings (accel, velocity, position) 1/(maxdev^2)
...
plot(TT(1:ii),XX(3,1:end-1),'-g');
subplot(3,1,2);hold on;
plot(TT(1:ii),UU,'-g');
subplot(3,1,3);hold on;
plot(TT(1:ii),corEst(3,:),'-g');



%Write gains to file
writeRedord(Ad,Bd1,Bd2,Cd,n,sampleTs,Kc,Lc1,'redordW');
